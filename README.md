...
- Plus the keyword `includeunit` (or `includeunits`) for measurement application values only, which appends the unit of measurement (for the measurement app the unit is read from CBus, _not_ the `unit=` keyword). Caution: This will make the sensor value a string, probably breaking any automation in HA that might expect a number, so using measurement app values without `includeunit` is probably what you want to be doing unless just displaying a value, which should probably use the right class anyway...

...
local cudRaw = {             -- All possible keywords for MQTT types, used in CUD function to exclude unrelated keywords for change detection
  ...
  'includeunit', 'includeunits', 'preset', 'noleveltranslate', 'exactpn', 'label', 'onoff'
}
...
local function addDiscover(net, app, group, channel, tags, name)
    ...
    if not is_boid then
      if special.label then
        ...
        if label[alias] ~= nil then
          attemptToDeleteTopic(mqttAttrTopic..oid)
          label[alias] = nil
        end
      end
    end
  ...
  if payload ~= nil and payload ~= 'buttons' and payload ~= 'inbound' then -- If payload received then publish
    ...
    if not oldLightingButton and lightingButton[alias] then -- If changing to a lighting button then clear status topics
      attemptToDeleteTopic(mqttReadTopic..alias..'/state')
      attemptToDeleteTopic(mqttReadTopic..alias..'/level')
    end
    return
...
local function cudAc()
  ...
  if removed > 0 then
    log('Removed '..removed..' Panasonic A/C object'..(removed ~= 1 and 's' or '')..(logms and ' in '..string.format('%.3f', socket.gettime()-nClock)..' seconds' or ''))
  end
  ...
  if addition then
    ...
    log('Added '..count..' Panasonic A/C object'..(count ~= 1 and 's' or '')..(logms and ' in '..string.format('%.3f', socket.gettime()-nClock)..' seconds' or ''))
  end
end
...
local function cudAt()
  ...
  if addition then log('AT object keywords added') end
end
    ...
    if not found[alias] then
      ...
      if (v.app == 202 or v.app == 203) and v.type == 'button' then
        for _, trigger in ipairs(v.trigger) do
          ...
          attemptToDeleteTopic(t)
        end
        if label[alias] ~= nil then
          attemptToDeleteTopic(label[alias])
          label[alias] = nil
        end
      else
        ...
        attemptToDeleteTopic(topic..'/state')
        attemptToDeleteTopic(topic..'/level')
        if v.type == 'select' then
          attemptToDeleteTopic(topic..'/select')
        end
        if label[alias] ~= nil then
          attemptToDeleteTopic(label[alias])
          label[alias] = nil
...
local function outstandingPublish()
  ...
  for _, u in ipairs(unpublished) do
    ...
    if u.app == 228 then
      ...
      if level ~= nil then stat, err = pcall(publishMeasurement, alias, u.net, u.group, u.channel, level) if not stat then log(err) end end
    -- User parameters
...
local function outstandingAtPublish()
  ...
  if #unpublishedAt > 0 then
    log('Published '..#unpublishedAt..' Airtopia discovery topic and current levels'..(#unpublishedAt ~= 1 and 's' or '')..(logms and ' in '..string.format('%.3f', socket.gettime()-nClock)..' seconds' or '')..(failed ~= 0 and ' ('..failed..' failed)' or ''))
    unpublishedAt = {}
  end
...
local function dupDelete()
  ...
  for toDelete, _ in pairs(discoveryDelete) do
    ...
    local oid = parts[1]
    local dType = parts[2]
    attemptToDeleteTopic(mqttDiscoveryTopic..dType..'/'..mqttDiscoveryNodeId..oid..'/config')
